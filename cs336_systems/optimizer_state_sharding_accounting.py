...
import time
from optimizer_state_sharding import ShardedOptimizer
from cs336_systems.transformer import Transformer

OUTPUT_DIR = "outputs/s6_optimizer_state_sharding_accounting"
...
def train(rank, world_size, use_sharded, num_steps=5):
    ...
    for step in range(num_steps):
        ...
        if step == 0:
            print(f"Rank {rank} | before optimizer step | {torch.cuda.memory_allocated() / 1024**3:.2f} GB")
        optimizer.step()
        if step == 0:
            print(f"Rank {rank} | after optimizer step | {torch.cuda.memory_allocated() / 1024**3:.2f} GB")
...
if __name__ == "__main__":
    ...
    for use_sharded in [False, True]:
        mp.spawn(train, args=(world_size, use_sharded), nprocs=world_size, join=True)